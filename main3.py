...
def process_single_ppt(ppt_file, output_dir, file_idx, total_files):
    ...
    # 构造调用参数
    call_params = {
        "function": "convert_images_to_webp_in_pptx",
    ...
    # 调用模块
    success, result = call_module("image_to_webp", params_json)

    if success:
        status, data = result
        if status.name == "SUCCESS":
            print(f"[{file_idx}/{total_files}] ✓ {ppt_file.name} 完成，转换WebP数量: {data}")
...
def process_all_ppt_files(max_concurrent=3):
    ...
    print(f"总共转换WebP: {total_converted} 个")
    print(f"{'='*50}")
